...
class BookingViewTestCase(TestCase):
    """Tests of the my_bookings view for students."""

    fixtures = ['tutorials/tests/fixtures/default_user.json']

    def setUp(self):
        #URL FOR STUDENT
        self.url = reverse('my_bookings') + "?user=student"
    ...
    def test_my_bookings_url(self):
        self.assertEqual(self.url, '/my_bookings/?user=student')
    ...
    def test_correct_booking_display(self): #only bookings with this account should be shown
        self.client.login(username=self.student.username, password='Password123')
        response = self.client.get(reverse('my_bookings'), {'user':'student', 'sortby':'date'})
        self.assertNotContains(response, 'JV') #in booking3, which isnt for this user

    def test_default_sorting(self):
        self.client.login(username=self.student.username, password='Password123')
        response = self.client.get(reverse('my_bookings'), {'user':'student', 'sortby':'date'}) #booking 1 should come before 2
        self.assertEqual(response.status_code, 200)
    ...
    def test_custom_sorting(self):
        self.client.login(username=self.student.username, password='Password123')
        response = self.client.get(reverse('my_bookings'), {'user':'student', 'sortby':'time'}) #booking 2 should come before 1
        self.assertEqual(response.status_code, 200)
